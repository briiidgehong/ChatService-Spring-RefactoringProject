	...
	public ArrayList<ChatDTO> getChatListByRecent(String fromID, String toID, int number) {
		...
		try { 
			...
			while (rs.next()) {
				...
				chat.setChatID(rs.getInt("chatID"));
				//sql injection / cross site script 같은 공격들을 방어하기 위해 특수문자들을 치환해준다.
				chat.setFromID(rs.getString("fromID").replaceAll(" ","&nbsp;").replaceAll(" ","&nbsp;").replaceAll("<","&lt").replaceAll(">","&gt").replaceAll("\n","<br>"));
				chat.setToID(rs.getString("toID").replaceAll(" ","&nbsp;").replaceAll(" ","&nbsp;").replaceAll("<","&lt").replaceAll(">","&gt").replaceAll("\n","<br>"));
				chat.setChatContent(rs.getString("chatContent").replaceAll(" ","&nbsp;").replaceAll(" ","&nbsp;").replaceAll("<","&lt").replaceAll(">","&gt").replaceAll("\n","<br>"));
				int chatTime = Integer.parseInt(rs.getString("chatTime").substring(11, 13));
				String timeType = "오전";
				if(chatTime >= 12) {
					timeType = "오후";
